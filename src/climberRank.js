function climbingLeaderboard(ranked, player) {
    let resultRanks = [];
    //cash ranks one time per method
    const setRanks = [...new Set(ranked)]; 
    //setRanks.sort((a,b)=>b-a);
    console.log(setRanks.length);
    //check if the player score is in the leader board
    player.forEach((e) => {

        //border cases
        if (e >= setRanks[0]) { return resultRanks.push(1); }
        else {
        for (let i = setRanks.length - 1; i >=0; i--) {
           if (e < setRanks[i]) return resultRanks.push(i+2);
           else if (e == setRanks[i]) return resultRanks.push(i+1);
            }
        }
